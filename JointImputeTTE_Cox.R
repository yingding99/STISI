...
# where the outcome Y is time-to-event with the substantive model being the Cox PH

STISI.yCox <- function(data.Orig,data.Boot,Y,Delta.Y,Y.cov,Z,V,Delta.Z,aft.cov,imp,TOL,trans){
  # data.Orig: the original dataset with Z censored 
  # data.Boot: one bootstrapped dataset (from data.Orig)
  # Y: Observed time-to-event data (min(T.Y,C.Y))
  # Delta.Y: censoring indicator for T.Y (1=observed, 0=censored)
  ...
  # Step 2: use complete bootstrap data to fit the substantive model and obtain the regression 
  # parameter estimates
  form <- "Surv(data.Boot[,Y],data.Boot[,Delta.Y])~"
  for(k in 1:length(Y.cov)) form <- paste0(form,"data.Boot[,Y.cov[",k,"]]+")
  Z.cov="Z"
  formula.cc <- as.formula(paste0(form,"data.Boot[,Z.cov]"))
  fit.cc <- coxph(formula.cc,subset=(data.Boot[,Delta.Z]==1))
  beta.cc <- summary(fit.cc)$coeff[,1] # beta coefficients (no intercept)
  BaseLambda.cc <- basehaz(fit.cc)
    ...
    if(id<dim(F.em)[1]) {
      ...
      while(U2>bdd){
        ...
        if(maxT.id<dim(F.cond.em)[1]){
          ...
          # compute the bound for time-to-event case
          linC <- as.numeric(data.Orig.cen[i,c(Y.cov,Z)]) %*% beta.cc
          id.Yobs <- sum(BaseLambda.cc[,2]<=as.numeric(data.Orig.cen[i,Y]))
          if(id.Yobs==0) id.Yobs=1
          cumHaz <- BaseLambda.cc[id.Yobs,1]
          
          expComp <- exp(linC)
          if(as.numeric(data.Orig.cen[i,Delta.Y])==0) bdd <- exp(-cumHaz*expComp)  
          if(as.numeric(data.Orig.cen[i,Delta.Y])==1) bdd <- exp(1+linC-cumHaz*expComp)*cumHaz
